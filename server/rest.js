  ...
  if (accessToken || process.env.NOAUTH) {
    ...
    if (typeof SiteStatistics === "object") {
      SiteStatistics.update({_id: "configuration"}, {$inc:{
        "Questionnaires.count.read": 1
      }});
    }

    var id = req.params.id;
    var questionnaireData = Questionnaires.findOne(id);
    delete questionnaireData._document;
    process.env.TRACE && console.log('questionnaireData', questionnaireData);
...
JsonRoutes.add("get", "/" + fhirVersion + "/Questionnaire", function (req, res, next) {
  ...
  if (accessToken || process.env.NOAUTH) {
    ...
    if (typeof SiteStatistics === "object") {
      SiteStatistics.update({_id: "configuration"}, {$inc:{
        "Questionnaires.count.search-type": 1
      }});
    }
    ...
    process.env.DEBUG && console.log('Questionnaires.find(id)', Questionnaires.find(databaseQuery).fetch());

    // because we're using BaseModel and a _transform() function
    // Questionnaires returns an object instead of a pure JSON document
    // it stores a shadow reference of the original doc, which we're removing here
    var questionnaireData = Questionnaires.find(databaseQuery).fetch();

    questionnaireData.forEach(function(patient){
