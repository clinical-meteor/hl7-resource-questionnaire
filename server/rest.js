...
JsonRoutes.add("put", "/" + fhirVersion + "/Questionnaire/:id", function (req, res, next) {
  process.env.DEBUG && console.log('PUT /fhir-3.0.0/Questionnaire/' + req.params.id);
  //process.env.DEBUG && console.log('PUT /fhir-3.0.0/Questionnaire/' + req.query._count);
  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      if (req.body) {
        ...
        personUpdate.resourceType = "Questionnaire";
        ...
        if(person){
          process.env.DEBUG && console.log('Questionnaire found...')
          personId = Persons.update({_id: req.params.id}, {$set: personUpdate },  function(error, result){
            if (error) {
              process.env.TRACE && console.log('PUT /fhir/Questionnaire/' + req.params.id + "[error]", error);
            ...
            if (result) {
              ...
              res.setHeader("Questionnaire", "fhir/Questionnaire/" + result);
...
JsonRoutes.add("get", "/" + fhirVersion + "/Questionnaire/:id", function (req, res, next) {
  process.env.DEBUG && console.log('GET /fhir-3.0.0/Questionnaire/' + req.params.id);
...
//==========================================================================================
// Step 3 - Update Questionnaire  

JsonRoutes.add("post", "/" + fhirVersion + "/Questionnaire", function (req, res, next) {
  process.env.DEBUG && console.log('POST /fhir/Questionnaire/', JSON.stringify(req.body, null, 2));
  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      if (req.body) {
        newQuestionnaire = req.body;


        // remove id and meta, if we're recycling a resource
        delete newQuestionnaire.id;
        delete newQuestionnaire.meta;


        newQuestionnaire = Persons.toMongo(newQuestionnaire);

        process.env.TRACE && console.log('newQuestionnaire', JSON.stringify(newQuestionnaire, null, 2));
        // process.env.DEBUG && console.log('newQuestionnaire', newQuestionnaire);

        console.log('Cleaning new person...')
        QuestionnaireSchema.clean(newQuestionnaire);

        var practionerContext = QuestionnaireSchema.newContext();
        practionerContext.validate(newQuestionnaire)
        console.log('New person is valid:', practionerContext.isValid());
        console.log('check', check(newQuestionnaire, QuestionnaireSchema))
        


        var personId = Persons.insert(newQuestionnaire,  function(error, result){
          if (error) {
          ...
          if (result) {
            process.env.TRACE && console.log('result', result);
            res.setHeader("Questionnaire", "fhir-3.0.0/Questionnaire/" + result);
            res.setHeader("Last-Modified", new Date());
            res.setHeader("ETag", "3.0.0");
...
//==========================================================================================
// Step 4 - QuestionnaireHistoryInstance

JsonRoutes.add("get", "/" + fhirVersion + "/Questionnaire/:id/_history", function (req, res, next) {
  process.env.DEBUG && console.log('GET /fhir-3.0.0/Questionnaire/', req.params);
  process.env.DEBUG && console.log('GET /fhir-3.0.0/Questionnaire/', req.query._count);
  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      persons.forEach(function(record){
        ...
        // the following is a hack, to conform to the Touchstone Questionnaire testscript
        // https://touchstone.aegis.net/touchstone/testscript?id=06313571dea23007a12ec7750a80d98ca91680eca400b5215196cd4ae4dcd6da&name=%2fFHIR1-6-0-Basic%2fP-R%2fQuestionnaire%2fClient+Assigned+Id%2fQuestionnaire-client-id-json&version=1&latestVersion=1&itemId=&spec=HL7_FHIR_STU3_C2
...
//==========================================================================================
// Step 5 - Questionnaire Version Read

// NOTE:  We've not implemented _history functionality yet; so this endpoint is mostly a duplicate of Step 2.

JsonRoutes.add("get", "/" + fhirVersion + "/Questionnaire/:id/_history/:versionId", function (req, res, next) {
  process.env.DEBUG && console.log('GET /fhir-3.0.0/Questionnaire/:id/_history/:versionId', req.params);
  //process.env.DEBUG && console.log('GET /fhir-3.0.0/Questionnaire/:id/_history/:versionId', req.query._count);
...
JsonRoutes.add("get", "/" + fhirVersion + "/Questionnaire", function (req, res, next) {
  process.env.DEBUG && console.log('GET /fhir-3.0.0/Questionnaire', req.query);
...
//==========================================================================================
// Step 6 - Questionnaire Search Type  

JsonRoutes.add("post", "/" + fhirVersion + "/Questionnaire/:param", function (req, res, next) {
  process.env.DEBUG && console.log('POST /fhir-3.0.0/Questionnaire/' + JSON.stringify(req.query));
...
//==========================================================================================
// Step 7 - Questionnaire Delete    

JsonRoutes.add("delete", "/" + fhirVersion + "/Questionnaire/:id", function (req, res, next) {
  process.env.DEBUG && console.log('DELETE /fhir-3.0.0/Questionnaire/' + req.params.id);
...
// WebApp.connectHandlers.use("/fhir/Questionnaire", function(req, res, next) {
//   res.setHeader("Access-Control-Allow-Origin", "*");
//   return next();
// });
